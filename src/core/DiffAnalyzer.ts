...
export class DiffAnalyzer {
    analyze(files: string[] = [], diffText: string = ""): DiffAnalysis {
...
function isTestLikeCode(line: string): boolean {
    return /\b(describe|it|test|expect|assert)\b/.test(line);
}

function collectTopLevelAreas(files: string[]): string[] {
    return [...new Set(files.map((file) => file.split("/")[0]!))];
}

function collectWorkspaceScopes(files: string[]): string[] {
    return [...new Set(
        files
            .map((file) => file.split("/"))
