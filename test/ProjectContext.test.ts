...
class TestProjectContext extends ProjectContext {
    ...
    readProjectPackageCalls: number;
    detectPackageManagerCalls: number;
...
test("initialize uses injected project package and package manager when provided", async () => {
    ...
    // Config now includes new system fields + legacy fields
    assert.deepEqual(context.config.skip, ["Debug Artifacts"]);
    assert.deepEqual(context.config.staged, { prettier: true, eslint: true });
    assert.deepEqual(context.config.ignore, []);
    assert.equal(context.config.autoPush, false);
    assert.deepEqual(context.config.risk, { failOn: null });
    assert.ok(context.config.rules);
    assert.ok(context.config.thresholds);
    assert.ok(context.config.weights);
});
...
test("initialize falls back to project lookups when injected values missing", async () => {
    ...
    // Config now includes new system fields + legacy fields
    assert.deepEqual(context.config.staged, { prettier: false, eslint: true });
    assert.deepEqual(context.config.skip, []);
    assert.deepEqual(context.config.ignore, []);
    assert.equal(context.config.autoPush, false);
    assert.deepEqual(context.config.risk, { failOn: null });
    assert.ok(context.config.rules);
