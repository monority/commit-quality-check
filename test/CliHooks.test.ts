...
const cliSource = join(repoRoot, "scripts", "cli.ts");
const tsxLoader = pathToFileURL(join(repoRoot, "node_modules", "tsx", "dist", "loader.mjs")).href;

function runCli(args: string[], opts: Options = {}) {
  // Use NODE_OPTIONS so --import is processed before cwd-relative resolution
  const env = {
    ...process.env,
    NODE_OPTIONS: `--import ${tsxLoader}`,
  };
  return execa("node", [cliSource, ...args], { ...opts, env });
}
...
test("auto-push command toggles post-commit hook and config", async () => {
  ...
  try {
    ...
    await runCli(["auto-push"], { cwd: root });
...
test("commit creates git commit from suggested header", async () => {
  ...
  try {
    ...
    const result = await runCli(["commit"], {
      cwd: root,
      reject: false,
    });
...
test("commit accepts explicit message override", async () => {
  ...
  try {
    ...
    const result = await runCli(["commit", "fix(src): custom message"], {
      cwd: root,
      reject: false,
    });
    const logResult = await execa("git", ["log", "-1", "--pretty=%s"], { cwd: root });
...
test("json-check supports full profile flag", async () => {
  ...
  try {
    ...
    const result = await runCli(["json-check", "--full"], {
      cwd: root,
      reject: false,
    });
    const payload = JSON.parse(result.stdout);
